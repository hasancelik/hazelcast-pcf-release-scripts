...
pushd $WORKSPACE
    ...
    pushd ./hazelcast-pcf-tile
        ...
        if [[ ! -v DEPLOYMENT_DISABLED ]]; then
            ...
            aws s3 cp ./product/hazelcast-pcf-${RELEASE_VERSION}.pivotal s3://$s3Bucket/partner-product-files/hazelcast/hazelcast-pcf-${RELEASE_VERSION}-test.pivotal --region $s3Region || { echo 'Could not upload .pivotal file to S3 Bucket!' ; exit 1; }
            ...
            TILE_PRODUCT_FILE_ID=`pivnet --format=json create-product-file --product-slug="hazelcast-pcf" --name="Hazelcast IMDG for PCF ${RELEASE_VERSION}" --aws-object-key="partner-product-files/hazelcast/hazelcast-pcf-${RELEASE_VERSION}-test.pivotal" --file-type='Software' --file-version=${RELEASE_VERSION} --sha256=${FILE_SHA256} --md5=${FILE_MD5} --docs-url=${DOCS_URL} | jq '.product_file.id'`
            ...
            if [[ ! -v DEPLOY_OSDF_MANUALLY ]]; then
                ...
                echo "Uploading .pivotal file to PivNet's S3 bucket..."
                aws s3 cp ./open_source_disclosures_Hazelcast_for_PCF-${RELEASE_VERSION}.txt s3://$s3Bucket/partner-product-files/hazelcast/open_source_disclosures_Hazelcast_for_PCF-${RELEASE_VERSION}-test.txt --region $s3Region

                echo "Adding OSDF file to PivNet..."
                OSDF_PRODUCT_FILE_ID=`pivnet --format=json create-product-file --product-slug="hazelcast-pcf" --name="Open Source Disclosures Hazelcast for PCF ${RELEASE_VERSION}" --aws-object-key="partner-product-files/hazelcast/open_source_disclosures_Hazelcast_for_PCF-${RELEASE_VERSION}-test.txt" --file-type='Open Source License' --file-version=${RELEASE_VERSION} | jq '.product_file.id'`
